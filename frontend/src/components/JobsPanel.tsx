...
const JobsPanel: React.FC<JobsPanelProps> = ({
  ...
  const [expandedPlotId, setExpandedPlotId] =
  ...
  const [autoFilledActivities, setAutoFilledActivities] = useState<
    any[]
  >([]);
  const [isNewFarmer, setIsNewFarmer] = useState(true);
  const [plotMode, setPlotMode] = useState<'existing' | 'new'>(
    'new',
  );
  const [selectedNewPlotId, setSelectedNewPlotId] = useState<
    number | null
  >(null);
  const [bookedActivities, setBookedActivities] = useState<any[]>(
    [],
  );
  const [notBookedActivities, setNotBookedActivities] = useState<
    any[]
  >([]);

  const [showNewFarmerModal, setShowNewFarmerModal] =
    useState(false);
  ...
  const [farmerAcresMap, setFarmerAcresMap] = useState<
  ...
  const loadFarmerPlots = async (farmerId: string) => {
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/plots/?farmer_id=${farmerId}&cluster_id=${clusterId}`,
      );
      const data = await res.json();
...
const [cluster, setCluster] = useState<Cluster | null>(null);
  ...
  const loadPlotJobs = async (plotId: number) => {
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/jobs/?plot=${plotId}`,
      );
      const data = await res.json();
      const filtered = data.map((job: any) => ({
        ...
        activities: (job.activities || []).filter(
          (a: any) =>
            Number(a.plot) === Number(plotId) ||
            Number(a.plot_id) === Number(plotId),
        ),
      }));
  ...
  const loadPlotActivityStatus = async (
    plotId: number,
    plotArea: number,
    farmerId: string,
  ) => {
  ...
  const autoFillFromCalendar = (
    ...
    const filled = clusterCalendar.map((cal, idx) => {
      ...
      d.setDate(
        d.getDate() + (idx === 0 ? 0 : cal.gap_days),
      );
      return {
  ...
  const recalcAmount = (list: any[]) => {
    const total = list.reduce(
      (s, a) =>
        s + (a.total_area || 0) * (a.rate_per_acre || 0),
      0,
    );
    setNewJob(prev => ({
      ...prev,
  ...
  const recalcFromNotBooked = (list: any[]) => {
    const total = list
      .filter(a => a.selected)
      .reduce(
        (s, a) => s + a.total_area * a.rate_per_acre,
        0,
      );
    setNewJob(prev => ({
      ...prev,
  ...
  const handleEditActivity = async () => {
    ...
    if (
      editingActivity.total_area <
      editingActivity.allocated_area
    ) {
      toast.error(
        `Cannot be less than allocated area (${editingActivity.allocated_area} ac)`,
      );
      return;
    }
    try {
      ...
      if (res.ok) {
        ...
        const plotId = editingActivity.plotId;
  ...
  const loadCluster = async () => {
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/clusters/${clusterId}/`,
      );
      if (!res.ok) return;
      const data = await res.json();
      setCluster(data);
  ...
  const [mukkadamOptions, setMukkadamOptions] = useState<
    { id: number; name: string; crew_size: number }[]
  >([]);
  const [farmerMukkadamMap, setFarmerMukkadamMap] = useState<
    Record<string, { id: number; name: string } | null>
  >({});
  const [assigningFarmerId, setAssigningFarmerId] =
    useState<string | null>(null);
    ...
    filtered.forEach(job => {
      if (!map.has(job.farmer_id)) {
        map.set(job.farmer_id, {
          ...
          plotCount: 0,
          totalAcres:
            Number(farmerAcresMap[job.farmer_id]) || 0,
          totalActivities: 0,
          remainingArea: 0,
          doneCount: 0,
          nearestDate: null,
          ...
          mobileNumber: (() => {
            const farmerData = farmers.find(
              (f: any) =>
                f.farmer_id === job.farmer_id,
            );
            const raw = farmerData?.phone_number || '';
            const str = String(raw);
            return str.startsWith('91') && str.length === 12
              ? str.slice(2)
              : str;
          })(),
      ...
      activities.forEach((a: any) => {
        if (
          ...
          if (
            !s.nearestDate ||
            a.scheduled_date < s.nearestDate
          ) {
      ...
      activities.forEach((a: any) => {
        ...
        const isFullyAllocated =
          Number(a.allocated_area) >=
          Number(a.total_area);
        const isPast =
          a.scheduled_date < todayStr;
        const isNextActivity =
          a.scheduled_date === s.nearestDate;
  ...
  <div>
    ...
    <p className="text-[10px] text-stone-400">
      {cluster?.village || ''}{cluster?.district ? ` · ${cluster.district}` : ''}
          ...
          <button
            type="button"
            className="text-xs text-stone-400 hover:text-emerald-600 flex items-center gap-0.5"
          >
            <ChevronDown
              size={12}
              className="rotate-90"
        ...
        <div className="flex items-center gap-1.5 mb-3 bg-stone-100 rounded-xl p-1">
          <button className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg bg-white text-[11px] font-semibold text-emerald-700 shadow-sm">
            Calendar
          </button>
          <button className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-medium text-stone-400">
            Payments
          </button>
          <button className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-medium text-stone-400">
            Insights
        ...
        {loading ? (
          ...
          <div className="jobs-list">
            {farmerSummaries.map(s => {
              const isExpanded =
                expandedFarmerId === s.farmerId;
              const plots =
                farmerPlots[s.farmerId] || [];
                  ...
                  {isExpanded && (
                    <div className="pt-2 px-3 pb-3 border-t border-emerald-200/60">
                      <div className="flex flex-col gap-2">
                        {plots.length === 0 ? (
                          <p className="text-xs text-stone-400 px-1 py-1.5">
                            No plots yet
                          </p>
                        ) : (
                          (() => {
                            const sortedPlots = [...plots].sort(
                              (a: any, b: any) => {
                                ...
                                const dB = getMeta(b);
                                if (
                                  dA.nearest !== dB.nearest
                                ) {
                                  if (!dA.nearest) return 1;
                                  if (!dB.nearest) return -1;
                                  return dA.nearest < dB.nearest
                                    ? -1
                                    : 1;
                                }
                            ...
                            return sortedPlots.map(
                              (plot: any) => {
                                const isPlotExpanded =
                                  expandedPlotId ===
                                  plot.id;
                                const currentPlotJobs =
                                  isPlotExpanded
                                    ? plotJobs[
                                        plot.id
                                      ] ||
                                ...
                                let plotNearestDate: string | null =
                                  null;
                                currentPlotJobs.forEach(
                                  (j: any) =>
                                    (
                                      j.activities ||
                                      []
                                  ...
                                  >
                                    ...
                                    >
                                      <div className="flex items-center gap-2 flex-1">
                                        ...
                                        <div>
                                          <p className="text-[13px] font-semibold text-stone-700 leading-tight">
                                            {plot.name}
                                          </p>
                                          {plot.crop_name && (
                                    ...
                                    {isPlotExpanded && (
                                      <div className="border-t border-stone-100">
                                        {currentPlotJobs.length ===
                                        0 ? (
                                          <p className="text-xs text-stone-400 px-3 py-2.5">
                                            No jobs
                                          </p>
                                        ) : (
                                          currentPlotJobs.map(
                                                        ...
                                                        >
                                                          <div>
                                                            <div className="flex items-center gap-1.5 mb-0.5">
                                                              ...
                                                              {isNextJob && (
                                                                <span className="text-[9px] font-semibold px-1.5 py-[1px] rounded bg-emerald-600 text-white tracking-[0.06em]">
                                                                  NEXT
                                                                </span>
                                                              )}
                                                              {isBacklog && (
                                                                <span className="text-[9px] font-semibold px-1.5 py-[1px] rounded bg-rose-100 text-rose-600 tracking-[0.06em]">
                                                                  BACKLOG
                                                                </span>
                                                              )}
                                                            </div>
