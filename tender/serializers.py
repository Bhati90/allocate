...
class FarmerSerializer(serializers.ModelSerializer):
    clusters    = serializers.PrimaryKeyRelatedField(many=True, read_only=True)
    ...
    class Meta:
        model  = Farmer
        fields = [
            'farmer_id', 'farmer_name', 'clusters', 'phone_number',
            'village', 'taluka', 'district', 'location',
            'latitude', 'longitude', 'total_acres',
        ]
        read_only_fields = ['village', 'taluka', 'district', 'total_acres']

    def get_total_acres(self, obj):
        # Uses prefetched plots — zero extra queries
        plots = obj.plots.all()
        total = sum(p.area_acres for p in plots if p.area_acres)
        return float(total)

    def _first_cluster(self, obj):
        # Uses prefetched clusters — zero extra queries
        clusters = obj.clusters.all()
        return clusters[0] if clusters else None

    def get_village(self, obj):
        c = self._first_cluster(obj)
        return c.village if c else ''

    def get_taluka(self, obj):
        c = self._first_cluster(obj)
        return c.taluka if c else ''

    def get_district(self, obj):
        c = self._first_cluster(obj)
        return c.district if c else ''
class JobActivitySerializer(serializers.ModelSerializer):
    ...
    moved_to_date = serializers.SerializerMethodField()
    allocations = serializers.SerializerMethodField()  # ← ADD

    def get_moved_to_date(self, obj):
        child = obj.moved_children.order_by('scheduled_date').first()
        return str(child.scheduled_date) if child else None

    def get_allocations(self, obj):  # ← ADD
        return [
            {
                'id':                alloc.id,
                'mukkadam':          alloc.mukkadam.mukkadam_id,
                ...
                'allows_second_job': alloc.allows_second_job,
                'is_carry_forward':  alloc.is_carry_forward,
            ...
            for alloc in obj.allocations.select_related('mukkadam').order_by('allocated_date')
        ]

    class Meta:
        model = JobActivity
        fields = [
            'id', 'activity_id', 'activity_name', 'activity_type',
            'is_strict', 'is_manually_moved', 'total_area', 'allocated_area',
            'remaining_area', 'crop_bundles', 'scheduled_date', 'scheduled_time',
            'estimated_workers', 'rate_per_acre', 'total_price',
            'transport_cost', 'other_cost', 'plot', 'plot_name',
