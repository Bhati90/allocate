...
# Dry run first: python manage.py sync_bookings_payments --ops-token YOUR_TOKEN --dry-run
...
# API status → local status mapping
STATUS_MAP = {
    'PAID':    'PAID',
    ...
    def handle(self, *args, **options):
        ...
        for job in local_jobs:
            ...
            with transaction.atomic():
                ...
                # ══════════════════════════════════════════════
                api_total    = round(float(api_booking.get('total_amount')  or 0), 2)
                api_advance  = round(float(api_booking.get('advance_paid')  or 0), 2)
                api_balance  = round(float(api_booking.get('balance')       or 0), 2)
                ...
                try:
                    ...
                    if round(float(booking.advance_paid or 0), 2) != api_advance:
                    ...
                    if round(float(booking.balance or 0), 2) != api_balance:
                    ...
                    if not dry_run:
                        ...
                        # dry run — create a temp object so payments loop works
                        booking = JobBooking(
                            job             = job,
                            booking_id      = api_book_id,
                            total_amount    = api_total,
                ...
                if dry_run or booking.pk:
                    ...
                    for pid, ap in api_pay_map.items():
                        api_amount = round(float(ap.get('amount') or 0), 2)
                        api_mode   = (ap.get('mode') or 'CASH').strip()
                        api_paid_at = parse_datetime(ap['paid_at']) if ap.get('paid_at') else None
                        from django.utils import timezone
                        if api_paid_at is None:
                            api_paid_at = timezone.now()
                        ...
                        if pid in local_pay_map:
                            ...
                            if round(float(lp.amount or 0), 2) != api_amount:
                            ...
                            self.stdout.write(
                                f'    [PAYMENT] CREATE id={pid} amount={api_amount} mode={api_mode}'
                            )
                            if not dry_run:
                                FarmerPayment.objects.create(
                                    ...
                                    notes      = api_notes,
                                    paid_at    = api_paid_at,
                                    paid_status= ap.get('paid_status', True),
                                )
                            payment_created += 1
