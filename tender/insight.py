...
class ClusterInsightsView(APIView):
    ...
    def get(self, request, cluster_id):
        ...
        # capacity
        capacity_by_day = get_effective_crew_size_for_cluster(
        ...
        crew_util_percent = (
            total_alloc_workers / effective_capacity_workers * 100
            if effective_capacity_workers
            else 0
        )
        slot_util_percent = (
            used_slots / total_slots * 100 if total_slots else 0
        )
        ...
        profit_per_acre = (
            float(profit) / float(total_area_allocated)
            if total_area_allocated
            else 0
        )
        ...
        while current <= end_date:
            ...
            cap_info = capacity_by_day.get(
                current,
                {"capacity_workers": 0, "total_slots": 0, "used_slots": 0},
            )
            ...
            day_profit = (
                (day_agg['total_farmer_amount'] or 0)
                - (day_agg['total_mukkadam_amount'] or 0)
            )
            ...
            by_day.append({
                ...
                "crew_utilization_percent": (
                    (day_agg['allocated_workers'] or 0) / eff_cap * 100
                    if eff_cap else 0
                ),
                "slot_utilization_percent": (
                    used_slots_day / total_slots_day * 100
                    if total_slots_day else 0
                ),
        ...
        for m in mukkadams_qs:
            ...
            m_profit = (
                (m_agg['farmer_amount'] or 0)
                - (m_agg['mukkadam_amount'] or 0)
            )
        ...
        # ---- by activity (per JobActivity summary) ----
        ...
        for act in activity_qs:
            ...
            a_agg = a_allocs.aggregate(
                ...
                loss_allocations=Count(
                    'id',
                    filter=Q(farmer_amount__lt=F('mukkadam_amount')),
                ),
            )
        ...
        while current <= end_date:
            cap_info = capacity_by_day.get(
                current, {"capacity_workers": 0, "total_slots": 0, "used_slots": 0}
            )
        ...
        for cd in capacity_demand:
            ...
            if candidate_days:
                candidate_days.sort(
                    key=lambda x: (-x[1], abs((x[0] - day).days))
                )
