...
def payment_overview(request):
    ...
    cluster_allocs = Allocation.objects.filter(
        ...
        'job_activity__job__farmer__farmer_name',
        'job_activity__job__farmer__phone_number',
    ...
    for row in cluster_allocs:
        ...
        g['farmer_name']  = row['job_activity__job__farmer__farmer_name']
        ...
        g['job_id']       = row['job_activity__job__job_id']

        if row['job_activity__plot__name']:
            g['plots'].add(row['job_activity__plot__name'])

        g['total_area'] += Decimal(str(row['allocated_area'] or 0))

        d = row['allocated_date']
    ...
    for (farmer_id, act_name), g in completed_groups.items():
        ...
        amount = (
            g['total_area'] * Decimal(str(g['rate']))
    ...
    for c in clusters:
        ...
        total_job_value = JobBooking.objects.filter(
            job_id__in=job_ids,
        ...
        total_collected = float(
            FarmerPayment.objects.filter(
                booking__job_id__in=job_ids,
                paid_status=True,
            ).aggregate(s=Sum('amount'))['s'] or 0
        )
        total_due = max(0, total_billed - total_collected)
        ...
        bills_awaiting = sum(1 for f in farmer_list if f['cluster_id'] == c.id and f['expected_payment'] == 'overdue')
    ...
    return Response({
        ...
        'pipeline': {
            ...
            'overdue': {
                ...
                'avg_days': round(
                    sum(f['days_since_billed'] or 0 for f in overdue_list) /
                    max(len(overdue_list), 1)
                ),
            },
            'collected_7d': {
                'count':  len(recent_payments),
                'amount': float(collected_7d),
            },
        },
        'forecast':        forecast,
...
class AllocationViewSet(viewsets.ModelViewSet):
    ...
    def change_date(self, request, pk=None):
        ...
        # ── Load full job sequence for sequencing checks ──────────────────────────
        # Done BEFORE the atomic block — no locks needed for read-only validation.
        ...
        try:
            with transaction.atomic():
                ...
                # Capacity warning included in response but not blocking (commented check preserved)
                ...
                if remaining_area_alloc > Decimal('0.01'):
                    ...
                    new_allocation = Allocation.objects.create(
                        job_activity      = job_act,   # re-pointed below if day_delta != 0
                        mukkadam          = mukkadam,
                        allocated_date    = new_date,
                ...
                if day_delta != 0 and original_date:
                    ...
                    new_job_activity = JobActivity.objects.create(
                        ...
                        original_gap_days       = gap_map.get(job_act.id, 0),
                    ...
                    # ── Cascade subsequent activities (Rule 3 + Rule 4) ──
                    #
                    # Reference starts at new_date (the moved split's date).
                    # - ALLOCATED successor  → stays fixed, becomes new reference anchor.
                    # - MANUALLY MOVED       → skip, do NOT update reference.
                    # - UNALLOCATED          → recalculate as reference_date + gap.
                    # - ZERO AREA            → skip.
                    #
                    # We exclude the original job_act and new_job_activity from cascade.

                    exclude_ids    = {job_act.id, new_job_activity.id}
                    reference_date = new_date   # start anchor = newly moved date
                    ...
                    for act in subsequent:
                        ...
                        act.scheduled_date = new_act_date
                        act.move_reason    = f'Cascade from allocation change_date — activity {job_act.id}'
                        if request.user and request.user.is_authenticated:
                            act.last_moved_by = request.user
                            act.last_moved_at = timezone.now()
                        act.save(update_fields=[
            ...
            for succ in successors:
                ...
                if succ.allocation_status in ('fully_allocated', 'partially_allocated', 'completed'):
                    ...
                    cascade_warnings.append({
                        ...
                        'new_date':      str(succ.scheduled_date),
                        'message':       (
                            f'Skipped — already {succ.allocation_status}. '
                            f'Becomes new reference anchor ({succ.scheduled_date}).'
                        ),
                    })
                    continue
                ...
                reference_date = new_succ_date  # advance anchor
...
def cluster_payment_dashboard(request, cluster_id):
    ...
    # ONE batch presign call for everything
    global_proof_map = get_presigned_urls_batch(list(set(all_s3_keys))) if all_s3_keys else {}

    # Build per-mukkadam lookups from pre-fetched querysets
    ...
    for farmer in farmers_in_cluster:
        ...
        for job in jobs:
            ...
            # Mukkadam from first allocation (no extra query — prefetched)
            job_mukkadam_name   = '—'
            job_mukkadam_mobile = '—'
            ...
            if booking:
                fps = list(booking.payments.all())  # prefetched
        ...
        farmer_data.append({
            ...
            'total_acres':   float(sum(
                ...
                for j in job_rows for act in j['activities']
                if act['total_area'] > 0
            )),
            'total_job_value': float(sum(Decimal(str(j['total_job_amount'])) for j in job_rows)),
            'total_plots':   len(set(
                act['plot_name'] for j in job_rows for act in j['activities']
            )),
    ...
    for _assignment in all_assignments:
        ...
        # Latest assignment for this mukkadam in this cluster
        assignment = max(
        ...
        # Group settlement payments by settlement id
        ...
        settlement_rows = []
        for s in settlements:
            # Allocations from prefetched job activities
            alloc_list = []
        ...
        # Transaction history from pre-fetched data
        txn_history = []
