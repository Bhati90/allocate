...
def calculate_used_slots_for_day(day_allocs):
    """
    Implement using your existing rule:
    - total slots per team per day = 2
    - allocation uses 1 or 2 slots depending on allows_second_job and same job_id grouping
    For now, simple version: each allocation uses 1 slot if allows_second_job, else 2.
    Replace with your real logic if you have it elsewhere.
    """
    used = 0
    for a in day_allocs:
        if a.allows_second_job:
            used += 1
        else:
            used += 2
    return used
...
def get_effective_crew_size_for_cluster(cluster, start_date, end_date):
    ...
    """
    result = {}
    mukkadams = Mukkadam.objects.filter(clusters=cluster).distinct()

    allocs = Allocation.objects.filter(
        cluster=cluster,
        allocated_date__range=[start_date, end_date]
    ).select_related('job_activity__job')
    ...
    while current <= end_date:
        ...
        total_slots = len(mukkadams) * 2  # or your own “available teams * 2”
